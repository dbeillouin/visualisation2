...
```{r  "re", message=FALSE, warning=FALSE}
...
```{r  "RE lme",message=FALSE, warning=FALSE}
library(nlme)
...
```{r  "RE lme4" ,message=FALSE, warning=FALSE}
library(lme4)
...
bayes_model <- brm(
  ...
  family = gaussian,
  prior = c(
    brms::prior(normal(0, 1), class = "Intercept"),   # Prior for the intercept
    brms::prior(cauchy(0, 1), class = "sd")            # Prior for the standard deviation of the random effect
  ),
  iter = 200,
  warmup = 100,
  cores = 4,
...
```{r  "metaforest", message=FALSE, warning=FALSE}
...
```{r  "ex1. 2", message=FALSE, warning=FALSE}
...
```{r  "ex1.3", message=FALSE, warning=FALSE}
...
```{r "boot1", message=FALSE, warning=FALSE}
...
```{r "boot2", message=FALSE, warning=FALSE}
# Load necessary libraries
library(metafor)
library(boot)
...
```{r "boot3", message=FALSE, warning=FALSE}
# Confidence intervals for mu
boot.ci(res.boot, type=c("norm", "basic", "stud", "perc"), index=1:2)
